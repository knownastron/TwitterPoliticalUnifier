...
class AWSConnection():
    ...
    def write_tweets(self, tweets):
        ...
        for tweet in tweets:
            cur.execute(sql, (tweet['tweet_id'], tweet['text'], Format.Format.format_username(tweet['user_screen_name']), tweet['created_at']))
        self.conn.commit()
    ...
    def get_searched_twitter_users(self, email):
        ...
        """
        sql = "SELECT * from SearchedTwitterUsers where Email = %s"
        cur = self.conn.cursor()
        cur.execute(sql, (email, ))
        twitter_users = cur.fetchall()
        return list(twitter_users)

    def insert_searched_twitter_user(self, email, screen_name):
        sql = "INSERT IGNORE INTO SearchedTwitterUsers (Email, ScreenName) VALUES (%s, %s)"
        cur = self.conn.cursor()
        cur.execute(sql, (email, screen_name, ))
        self.conn.commit()
